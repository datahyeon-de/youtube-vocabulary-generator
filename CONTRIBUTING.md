...
`.cursor/commands/commands.yaml`에 정의된 명령은 프로젝트 계획과 문서를 일관되게 유지하는 데 사용됩니다. 커서를 사용할 때 아래 명령을 참고하세요.

| 명령 | 설명 | 주요 동작 |
|------|------|-----------|
| `init_project` | 초기 폴더 및 템플릿 구조 생성 | `.cursor`, `.github` 템플릿, 기본 파일 생성 |
| `show_plan` | 현재 프로젝트 계획 확인 | `.cursor/project-plan.md` 출력, 세션 요약 업데이트 |
| `sync_plan` | 계획 상태 동기화 | 체크박스 상태 반영 후 계획 업데이트 |
| `current_phase` | 진행 중인 Phase 확인 | 현재 Phase/Task 파악 및 다음 단계 안내 |
| `next_phase` | 다음 Phase로 진행 표시 | 계획 파일 상태 갱신, 요약 기록 |
| `next_task` | 다음 Task로 진행 표시 | 현 Task 업데이트, 요약 기록 |
| `update_readme` | README 동기화 | 진행 내역을 README에 반영 후 요약 기록 |

- 명령 실행 후에는 변경 사항이 자동 반영되므로 PR 생성 전에 `.cursor/project-plan.md`와 README 변경분을 확인하세요.
- 새로운 규칙이 생기면 `rules/rules.mdc`에 기록하고 필요 시 커서 명령을 추가하세요.
