...
- **주요 기능**:
  ...
  - 단어장 생성 및 제공 API
- **기술 스택**: Python, FastAPI, Pydantic, Starlette Middleware, pytest, youtube-transcript-api(도입 예정), LLM API(도입 예정)
...
- ⏳ `get_transcript(video_id)` 서비스 구현 (`app/services/transcript.py` 예정)
...
- ◻️ 자막 기반 핵심 단어 추출 로직 구현
- ◻️ 한국어 의미 생성 및 도메인 분류 로직 정립
- ◻️ 모듈 단위 테스트 및 예외 처리
...
- 완료: Phase 1, Phase 2, 로깅 미들웨어 추가 작업
- 진행 중: Phase 3 구현 (자막 서비스 로직 및 엔드포인트 개발)
- 예정: Phase 4 이후 LLM 및 단어장 처리, 통합 테스트 단계
